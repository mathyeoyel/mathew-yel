    ...
    <main className="mx-auto max-w-4xl px-5 py-20">
      ...
      {profile.currentFocus?.length ? (
        <section className="mt-14">
          <h2 className="text-2xl font-black text-brand-deep">Current focus</h2>
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {profile.currentFocus.map((item) => (
              <div
                key={item}
                className="rounded-sm border border-brand-border-light bg-white p-5 text-sm font-semibold text-brand-body"
              >
                {item}
              </div>
            ))}
